...
from archive_paths import activate as activate_shared_runtime

activate_shared_runtime()

from experiments.place_wallis_benchmark.training_utils import average_state_dicts
...
from zramp_runtime import support_expert_bank as core
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self._weighted_selection == "grid-intensity":
            self._communication_assumptions.update({
                "pull_rule": "highest advertised summed grid intensity",
            ...
            for sender in sorted(neighbours[receiver]):
                ...
                for key in sender_keys:
                    if (
                        (
                            self._weighted_acquisition
                            or self._weighted_selection == "grid-intensity"
                        )
    ...
    def _save_checkpoint(self, step: int) -> None:
        ...
        if self._weighted_selection == "grid-intensity":
            status["weighted_single_model"].update({
                "selection": "advertised summed grid intensity",
