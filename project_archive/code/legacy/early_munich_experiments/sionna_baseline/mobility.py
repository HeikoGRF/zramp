...
class RandomWaypoint:
    ...
    """
    def __init__(self, bounds_x=(-400, 400), bounds_y=(-400, 400), velocity=(1.0, 3.0), pause_time=2.0):
    ...
    def _get_random_destination(self):
        return np.array([
            np.random.uniform(*self.bounds_x),
            np.random.uniform(*self.bounds_y)
        ])
...
def get_anchor_zone(x, y, square_size=200):
    """
    Maps an (x, y) coordinate to a non-overlapping 200x200 Square Anchor Zone.
    Returns a string indentifier for the zone.
    """
    zone_x = int(np.floor(x / square_size))
    zone_y = int(np.floor(y / square_size))
