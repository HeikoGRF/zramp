"""
Greedy flooding baseline using the Neural Radio Map + LoRA adapters
in a simple empty scene (no buildings), for fair comparison.

Differences to main_baseline_greedy_lora.py:
- Uses an empty Sionna Scene (free space), not the Munich map.
- No building grid / obstruction features.
- Same pre-training, adapter logic, and logging structure so results
...
from sionna.rt import PlanarArray, Transmitter, Receiver, PathSolver, Scene
...
class LoRAZoneAdapter:
    ...
    def __init__(self, map_cfg: MapConfig, noise_floor_db: float = -150.0, lr: float = 1e-3,
        ...
        self.max_buffer_size = 6000

    @property
    def n_uncensored(self) -> int:
        return sum(1 for _, r in self.buffer if r != self.noise_floor_db)

    def add(self, tx_xy: np.ndarray, rx_xy: np.ndarray, rssi: float) -> None:
        feats = self.encoder.encode(tuple(tx_xy), tuple(rx_xy), tx_power_dbm=0.0)
    ...
    def predict(self, tx_xy: np.ndarray, rx_xy: np.ndarray) -> Tuple[float, float]:
        feats = self.encoder.encode(tuple(tx_xy), tuple(rx_xy), tx_power_dbm=0.0).unsqueeze(0).to(self.device)
        self.model.eval()
        with torch.no_grad():
...
def main():
    print("Creating empty scene …", flush=True)
    scene = Scene()
    freq_hz: float = 3.5e9
    ...
    zone_size = 100
    bounds_x = (0, zone_size - 1)
    bounds_y = (0, zone_size - 1)
    ...
    print(f"Single anchor zone (empty scene): Zone_0_0 (bounds {bounds_x}, {bounds_y})", flush=True)
    ...
    for step in range(num_steps):
        ...
        for tx_idx in range(num_nodes):
            ...
            for rx_idx in range(num_nodes):
                ...
                zone = get_anchor_zone(pos_rx[0], pos_rx[1], square_size=zone_size)

                adapter = node_adapters[rx_idx].setdefault(
                    zone, LoRAZoneAdapter(map_cfg, fspl_const=fspl_const, base_state=base_state)
                )

                pred_mean, pred_sigma = adapter.predict(tx_xy, rx_xy)
    ...
    log_file = "simulation_log_greedy_lora_empty.csv"
    share_file = "greedy_flooding_shares_lora_empty.csv"
    summary_file = "greedy_flooding_summary_lora_empty.txt"
    rmse_file = "step_rmse_lora_empty.csv"
    ...
    with open(summary_file, "w") as f:
        f.write("Greedy Flooding Baseline (MLP + LoRA, Empty Scene) Summary\n")
        f.write("=" * 65 + "\n")
    ...
    print("\n✓ Greedy flooding with MLP+LoRA adapters on empty scene completed.")
