#!/usr/bin/env python3
"""Build role routes for the two-community 150 m urban scenario."""
...
SCENARIO_NAME = "single_zone_urban_150_communities"
SCENARIO = ROOT / "SUMO" / SCENARIO_NAME
DEFAULT_NET = SCENARIO / f"{SCENARIO_NAME}.net.xml"

GATES = {
    "S000": {"entry": "N0_0__N0_1", "exit": "N0_1__N0_0", "district": "south"},
    "S035": {"entry": "N1_0__N1_1", "exit": "N1_1__N1_0", "district": "south"},
    "S075": {"entry": "N2_0__N2_1", "exit": "N2_1__N2_0", "district": "south"},
    "S115": {"entry": "N3_0__N3_1", "exit": "N3_1__N3_0", "district": "south"},
    "S150": {"entry": "N4_0__N4_1", "exit": "N4_1__N4_0", "district": "south"},
    "N000": {"entry": "N0_5__N0_4", "exit": "N0_4__N0_5", "district": "north"},
    "N035": {"entry": "N1_5__N1_4", "exit": "N1_4__N1_5", "district": "north"},
    "N075": {"entry": "N2_5__N2_4", "exit": "N2_4__N2_5", "district": "north"},
    "N115": {"entry": "N3_5__N3_4", "exit": "N3_4__N3_5", "district": "north"},
    "N150": {"entry": "N4_5__N4_4", "exit": "N4_4__N4_5", "district": "north"},
    "W045": {"entry": "N0_1__N1_1", "exit": "N1_1__N0_1", "district": "south"},
    "W105": {"entry": "N0_4__N1_4", "exit": "N1_4__N0_4", "district": "north"},
    "E045": {"entry": "N4_1__N3_1", "exit": "N3_1__N4_1", "district": "south"},
    "E105": {"entry": "N4_4__N3_4", "exit": "N3_4__N4_4", "district": "north"},
}
...
def _shortest_route(net, entry_gate: str, exit_gate: str) -> list[str]:
    start = net.getEdge(str(GATES[entry_gate]["entry"]))
    stop = net.getEdge(str(GATES[exit_gate]["exit"]))
    result = net.getShortestPath(start, stop)
...
def _choose_visitor_trip(
    rng: random.Random, *, same_district_probability: float
) -> tuple[str, str]:
    names = sorted(GATES)
    entry = rng.choice(names)
    local = [
        gate
        for gate in names
        if gate != entry
        and GATES[gate]["district"] == GATES[entry]["district"]
    ]
    remote = [
        gate
        for gate in names
        if GATES[gate]["district"] != GATES[entry]["district"]
    ]
    pool = local if rng.random() < same_district_probability else remote
...
def _build_config(path: Path, *, net: Path, routes: Path, polygons: Path) -> None:
...
def build(
    *,
    ...
    num_vehicles: int,
    regular_count: int,
    same_district_probability: float,
) -> None:
    ...
    if not 0.0 <= same_district_probability <= 1.0:
    ...
    for node_idx in range(regular_count, num_vehicles):
        entry, exit_gate = _choose_visitor_trip(
            rng,
            same_district_probability=same_district_probability,
        )
    ...
    plan = {
        "format": "single_zone_community_role_route_plan_v1",
        "seed": int(seed),
        "map": SCENARIO_NAME,
        "map_size": 150.0,
        ...
        "visitor_replacement": {
            ...
            "exit_gate": "district_biased_random_over_other_border_intersections",
            "same_district_probability": float(same_district_probability),
...
def main() -> int:
    ...
    parser.add_argument(
        "--polygons",
        type=Path,
        default=SCENARIO / f"{SCENARIO_NAME}.poly.xml",
    )
    parser.add_argument("--routes", type=Path, required=True)
    parser.add_argument("--plan", type=Path, required=True)
    parser.add_argument("--config", type=Path, required=True)
    parser.add_argument("--seed", type=int, default=1)
    parser.add_argument("--num-vehicles", type=int, default=40)
    parser.add_argument("--regular-count", type=int, default=4)
    parser.add_argument("--same-district-probability", type=float, default=0.8)
    ...
    build(
        ...
        same_district_probability=args.same_district_probability,
    )
