#!/usr/bin/env bash
# Submit the learned-acquisition expert-bank sweep on the prepared 3x3
# building-density x traffic-density Luxembourg zones.
...
submission_table=${SUBMISSION_TABLE:-${RESULTS_ROOT}/submitted_pq4x256_kappa_sweep_jobs.tsv}
printf 'zone\tbuilding_score\tvehicle_score\tpenalty_pct\tjob_id\tresults_dir\n' \
    > "${submission_table}"
...
for crop in "${ZONES[@]}"; do
    ...
    for penalty_pct in "${PENALTIES[@]}"; do
        ...
        job_id=$(env SLURM_CONF="${SLURM_CONFIG}" sbatch --parsable \
            --job-name="${short}-pq-k${penalty_pct}" \
            --output="${results_dir}/slurm-%j.out" \
            --error="${results_dir}/slurm-%j.err" \
            --export="ALL,PYTHONUNBUFFERED=1,TRACE_PATH=${trace},TESTSET_PATH=${testset},NET_PATH=${net},RESULTS_DIR=${results_dir},BANK_CAPACITY=6,LEARNED_ACQUISITION_BUNDLE=${BUNDLE},ACQUISITION_RELATIVE_GAIN_PENALTY=${penalty},REPLAY_CAPACITY=0,FULL_DATASET_EPOCHS=1,PLANE_ANGLE_DEG=12,PLANE_INITIAL_HALF_WIDTH_M=0,CHECKPOINT_EVERY=50,TAIL_EVAL_COUNT=10,TAIL_EVAL_STRIDE=25,METHOD_TAG=${method_tag}" \
            "${SIM_SCRIPT}")
        printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
            "${crop}" "${building_score}" "${vehicle_score}" \
            "${penalty_pct}" "${job_id}" "${results_dir}" \
            >> "${submission_table}"
        submitted=$((submitted + 1))
    done
done

printf 'Submitted %d simulations. Manifest: %s\n' \
