#!/usr/bin/env python3
"""Extensive synthetic pretraining for one scalar acquisition gain."""
...
DEFAULT_OUTPUT = (
    ROOT
    / "artifacts/support_acquisition_pretraining/synthetic_scalar_gain_v1"
)
...
class ScalarBatch:
    ...
    def to(self, device: torch.device) -> "ScalarBatch":
        return ScalarBatch(**{
...
def make_batch(
    ...
    def append_expert(rows: np.ndarray) -> int:
        nonlocal expert_index
    ...
    for group in range(int(group_count)):
        ...
        bank_indices = [append_expert(rows) for rows in bank_rows]
        ...
        for _candidate in range(cfg.candidates_per_bank):
            ...
            candidate_indices.append(append_expert(rows))
            bank_rows_out.append(bank_indices)
            targets.append(target)
            group_indices.append(group)

    maximum_bank = max(map(len, bank_rows_out))
    padded_bank = np.zeros(
        (len(bank_rows_out), maximum_bank), dtype=np.int64
    )
    bank_mask = np.zeros_like(padded_bank, dtype=bool)
    ...
    return ScalarBatch(
        ...
        expert_scales=torch.from_numpy(
...
def save_best(
    ...
    *,
    ...
    torch.save(
        {
            "format": "scalar_plane_set_encoder_v1",
    ...
    torch.save(
        {
            "format": "scalar_support_gain_model_v1",
    ...
    torch.save(
        {
            "format": "synthetic_scalar_support_gain_bundle_v1",
...
def self_test() -> None:
    ...
    acquisition = ScalarGainModel(latent_dim=16, hidden_dim=32)
...
def main() -> int:
    ...
    self_test()
    if args.self_test:
        print("scalar acquisition pretraining self-test passed")
        return 0
    ...
    for step in range(1, int(args.max_steps) + 1):
        ...
        if (
            ...
            if improved:
                ...
                stale_validations += 1
            print(
                f"[SCALAR-ACQ] step={step:05d}/{args.max_steps} "
    ...
    atomic_json(output / "metrics.json", {
        "schema": "synthetic_scalar_support_gain_pretraining_v1",
        "status": "complete",
        "map_data_used": False,
        ...
        "architecture": {
            ...
            "bank_encoder": "shared token MLP plus mean/max pooling",
    ...
    print(
        f"[SCALAR-ACQ] saved={output} best_step={best_step} "
