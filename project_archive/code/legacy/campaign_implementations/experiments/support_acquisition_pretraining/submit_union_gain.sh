#!/usr/bin/env bash
#SBATCH --job-name=union-support-gain
...
OUTPUT_DIR=${OUTPUT_DIR:-${REPO_ROOT}/artifacts/support_acquisition_pretraining/synthetic_union_gain_v1_excessive}
...
cd "${REPO_ROOT}"
"${PYTHON_BIN}" experiments/support_acquisition_pretraining/pretrain_union_gain.py \
    --output "${OUTPUT_DIR}" \
    --seed "${SEED:-20260810}" \
