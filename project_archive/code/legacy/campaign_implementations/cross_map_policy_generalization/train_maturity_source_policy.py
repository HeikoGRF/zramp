#!/usr/bin/env python3
"""Pretrain an exact policy to select mature providers on artificial maps."""
...
def maturity_dataset(directory: Path, dataset):
    targets = {}
    grouped = {}
    with (directory / "parameter_objective_audit.csv").open(
        ...
        for row in csv.DictReader(handle):
            key = (
                int(row["step"]), str(row["mode"]),
                int(row["receiver_idx"]), int(row["provider_idx"]),
            )
            grouped.setdefault(key, float(row["provider_maturity"]))
    examples = [
        replace(
            row,
            oracle_best_gain=grouped[(
                int(row.step), str(row.mode),
                int(row.receiver), int(row.provider),
            )],
        )
        for row in dataset.examples
    ]
    return type(dataset)(
        dataset.states, examples, dataset.group_widths, dataset.trajectory_dim
    )
...
def load_cases(root: Path, split: str, case_start: int):
    ...
    for audit in sorted(root.glob(f"{pattern}/seed_*/audit")):
        ...
        dataset = maturity_dataset(audit, load_seed(audit, case_id))
...
def main() -> int:
    ...
    checkpoint = {
        ...
        "training_target": "provider-maturity",
        "source_state_process": "random plus policy-induced artificial deployment evolution",
    ...
    report = {
        "format": "maturity_source_policy_report_v1",
        "protocol": {
            "fit_and_selection": "artificial maps only",
            "deployment_map_used": False,
            "random_training_included": not bool(args.no_random_training),
            "onpolicy_repeats": repeats,
            "training_target": "sample-count-free provider parameter maturity",
        },
        "seed": int(args.seed),
        "best_epoch": int(best_epoch),
        "train_pairs": len(training.examples),
        "validation_pairs": len(validation.examples),
        "best_validation_uplift": float(validation_metric["gain_over_random_db"]),
